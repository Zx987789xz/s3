...
namespace ChatCoreTest
{
    internal class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            Console.WriteLine(" i: " + ii + " f: " + f + " str: " + str);
            Console.Read();
        ...
        private static bool Read(out string str)
        {
            ...
            if (BitConverter.IsLittleEndian)
            {
                var byteDate = new byte[length];
                Buffer.BlockCopy(m_PacketData, (int)m_Pos, byteDate, 0, length);
                Array.Reverse(byteDate);
                str = Encoding.Unicode.GetString(byteDate, 0, length);
            }
            else
            {
